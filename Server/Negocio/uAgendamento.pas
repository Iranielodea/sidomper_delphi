...
uses uDM, System.SysUtils, FireDAC.Stan.Param,
  ...
  System.Variants, uFiltroAgendamento, uFireDAC, System.Generics.Collections;
...
type
  ...
  public
    ...
    function ListarQuadro(AQuery: TFDQuery): TObjectList<TAgendamentoVO>;
...
begin
  ...
  try
    ...
    if obj.Model.FieldByName('Age_Status').AsInteger = StrToInt(sIdStatus) then
      raise Exception.Create('Agendamento já encerrado!');
...
function TAgendamento.ListarQuadro(
  AQuery: TFDQuery): TObjectList<TAgendamentoVO>;
var
  model: TAgendamentoVO;
  Lista: TObjectList<TAgendamentoVO>;
begin
  Lista := TObjectList<TAgendamentoVO>.Create();
  AQuery.First;
  while not AQuery.Eof do
  begin
    model := TAgendamentoVO.Create;
    model.Id := AQuery.FieldByName('Age_Id').AsInteger;
    model.Data := AQuery.FieldByName('Age_Data').AsDateTime;
    model.Hora := AQuery.FieldByName('Age_Hora').AsDateTime;
    model.IdCliente := AQuery.FieldByName('Age_Cliente').AsInteger;
    model.NomeCliente := AQuery.FieldByName('Age_NomeCliente').AsString;
    model.Usuario.Nome := AQuery.FieldByName('Usu_Nome').AsString;
    model.Status.Nome := AQuery.FieldByName('Sta_Nome').AsString;
    AQuery.Next;
    Lista.Add(model);
  end;
  Result := Lista;
