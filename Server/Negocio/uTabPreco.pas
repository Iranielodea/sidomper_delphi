...
interface
  ...
  const CConsulta: string =
    'SELECT Tab_Id, Tab_Nome, Tab_Referencia FROM TabPreco ';
...
//  TTabPreco = class
  public
    ...
    function Filtrar(Campo, Texto, Ativo: string; AFiltro: TTabPrecoFiltro; Contem: Boolean = True; Id: Integer = 0): string; overload;
...
function TTabPreco.Filtrar(Campo, Texto, Ativo: string; AFiltro: TTabPrecoFiltro; Contem: Boolean = True; Id: Integer = 0): string;
var
  sConsulta: string;
  sTexto: string;
begin
  ...
  if Id > 0 then
    ...
    if StrToIntDef(AFiltro.TabelaId, 0) > 0 then
      sConsulta := sConsulta + ' AND Tab_Id = ' + AFiltro.TabelaId;

    if AFiltro.ProdutoId <> '' then
      sConsulta := sConsulta + ' AND Tab_Produto IN ' + AFiltro.ProdutoId;

    if AFiltro.StatusId <> '' then
      sConsulta := sConsulta + ' AND Tab_Status IN ' + AFiltro.StatusId;

    if AFiltro.TipoId <> '' then
      sConsulta := sConsulta + ' AND Tab_Tipo IN ' + AFiltro.TipoId;
