...
type
  TDMSolicitacao = class(TDMBase)
    ...
    CDSOcorrenciaRegraConsulta: TClientDataSet;
    CDSOcorrenciaRegraConsultaSOcor_Anexo: TStringField;
    CDSOcorrenciaRegraConsultaSOcor_Classificacao: TIntegerField;
    CDSOcorrenciaRegraConsultaSOcor_Data: TDateField;
    CDSOcorrenciaRegraConsultaSOcor_Descricao: TMemoField;
    CDSOcorrenciaRegraConsultaSOcor_Hora: TTimeField;
    CDSOcorrenciaRegraConsultaSOcor_Id: TAutoIncField;
    CDSOcorrenciaRegraConsultaSOcor_Solicitacao: TIntegerField;
    CDSOcorrenciaRegraConsultaSOcor_Tipo: TIntegerField;
    CDSOcorrenciaRegraConsultaSOcor_UsuarioOperacional: TIntegerField;
    CDSOcorrenciaRegraConsultaTip_Codigo: TIntegerField;
    CDSOcorrenciaRegraConsultaTip_Nome: TWideStringField;
    CDSOcorrenciaRegraConsultaUsu_Codigo: TIntegerField;
    CDSOcorrenciaRegraConsultaUsu_Nome: TWideStringField;
    CDSCadastroSol_Categoria: TIntegerField;
    CDSCadastroCat_Codigo: TIntegerField;
    CDSCadastroCat_Nome: TStringField;
    CDSListarProblemaSolucao: TClientDataSet;
    CDSListarProblemaSolucaoSol_Id: TAutoIncField;
    CDSListarProblemaSolucaoSol_Data: TDateField;
    CDSListarProblemaSolucaoSol_Hora: TTimeField;
    CDSListarProblemaSolucaoSol_Descricao: TMemoField;
    CDSListarProblemaSolucaoSol_DescricaoLiberacao: TMemoField;
    CDSListarProblemaSolucaoDescricaoOco: TMemoField;
    CDSListarProblemaSolucaoSOcor_Classificacao: TIntegerField;
    CDSListarProblemaSolucaoDataOco: TDateField;
    CDSListarProblemaSolucaoHoraOco: TTimeField;
    ...
    procedure CDSOcorrenciaTecnicaConsultaSOcor_HoraSetText(Sender: TField; const
        Text: string);
    procedure CDSOcorrenciaRegraConsultaSOcor_DataSetText(Sender: TField;
      const Text: string);
    procedure CDSOcorrenciaRegraConsultaSOcor_HoraSetText(Sender: TField;
      const Text: string);
    procedure CDSOcorrenciaRegraConsultaBeforePost(DataSet: TDataSet);
    procedure CDSOcorrenciaRegraConsultaNewRecord(DataSet: TDataSet);
    procedure CDSOcorrenciaRegraConsultaReconcileError(
...
begin
  ...
  if Trim(CDSOcorrenciaGeralConsultaSOcor_Descricao.asstring) = '' then
    raise Exception.Create('Informe a Descrição da Ocorrência Geral.');
  ...
  if CDSOcorrenciaGeralConsultaSOcor_UsuarioOperacional.AsInteger = 0 then
    raise Exception.Create('Informe o Operador da Ocorrência Geral.');

//  if CDSOcorrenciaGeralConsultaSOcor_Tipo.AsInteger = 0 then
//    raise Exception.Create('Informe o Tipo da Ocorrência Geral.');
...
//      raise Exception.Create('Arquivo do Anexo Ocorrência Geral não Existe.');
//  end;
...
procedure TDMSolicitacao.CDSOcorrenciaRegraConsultaBeforePost(
  DataSet: TDataSet);
begin
  inherited;
  if CDSOcorrenciaRegraConsultaSOcor_UsuarioOperacional.AsInteger = 0 then
    raise Exception.Create('Informe o Operador da Ocorrência Regra.');

  if Trim(CDSOcorrenciaRegraConsultaSOcor_Descricao.asstring) = '' then
    raise Exception.Create('Informe a Descrição da Ocorrência Regra.');

//  if CDSOcorrenciaTecnicaConsultaSOcor_Tipo.AsInteger = 0 then
//    raise Exception.Create('Informe o Tipo da Ocorrência Técnica.');

//  if Trim(CDSOcorrenciaRegraConsultaSOcor_Anexo.asstring) <> '' then
//  begin
//    if not FileExists(CDSOcorrenciaRegraConsultaSOcor_Anexo.asstring) then
//      raise Exception.Create('Arquivo do Anexo Ocorrência Regras não Existe.');
//  end;

  if CDSOcorrenciaRegraConsultaSOcor_Hora.AsString <> '' then
    CDSOcorrenciaRegraConsultaSOcor_Hora.AsString := FormatDateTime('hh:mm', CDSOcorrenciaRegraConsultaSOcor_Hora.AsDateTime);

  if CDSOcorrenciaRegraConsulta.State = dsInsert then
  begin
    CDSOcorrenciaRegraConsultaSOcor_Id.AsInteger := FContadorRegra;
    Dec(FContadorRegra);
  end;
end;

procedure TDMSolicitacao.CDSOcorrenciaRegraConsultaNewRecord(DataSet: TDataSet);
begin
  inherited;
  CDSOcorrenciaRegraConsultaSOcor_Solicitacao.AsInteger := -1;
  CDSOcorrenciaRegraConsultaSOcor_Classificacao.AsInteger := 3;
  CDSOcorrenciaRegraConsultaSOcor_Data.AsDateTime := Date;
  CDSOcorrenciaRegraConsultaSOcor_Hora.AsDateTime := Time;

  Dec(FContadorRegra);
end;

procedure TDMSolicitacao.CDSOcorrenciaRegraConsultaReconcileError(
  DataSet: TCustomClientDataSet; E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
  inherited;
...
procedure TDMSolicitacao.CDSOcorrenciaRegraConsultaSOcor_DataSetText(
...
procedure TDMSolicitacao.CDSOcorrenciaRegraConsultaSOcor_HoraSetText(
  Sender: TField; const Text: string);
begin
  inherited;
  TFuncoes.ValidaHora(Sender, Text);
...
begin
  ...
  if CDSOcorrenciaTecnicaConsultaSOcor_UsuarioOperacional.AsInteger = 0 then
    raise Exception.Create('Informe o Operador da Ocorrência Técnica.');

  if Trim(CDSOcorrenciaTecnicaConsultaSOcor_Descricao.asstring) = '' then
    raise Exception.Create('Informe a Descrição da Ocorrência Técnica.');
...
//      raise Exception.Create('Arquivo do Anexo Ocorrência Técnica não Existe.');
//  end;
