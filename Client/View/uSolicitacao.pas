...
begin
  ...
  if FController.Model.CDSConsulta.IsEmpty then
    raise Exception.Create('Informe a Solicitação!');
...
begin
  ...
  if dsCronograma.State in [dsEdit, dsInsert] then
    raise Exception.Create('Cronograma está em Modo de Edição, Salve ou Cancele!');

  if dsOcorrenciaGeral.State in [dsEdit, dsInsert] then
    raise Exception.Create('Ocorrência Geral está em Modo de Edição, Salve ou Cancele!');

  if dsOcorrenciaTecnica.State in [dsEdit, dsInsert] then
    raise Exception.Create('Ocorrência Técnica está em Modo de Edição, Salve ou Cancele!');
  ...
  begin
    if (tsAnalista.TabVisible) and (FController.Model.CDSCadastroSol_VersaoId.AsInteger = 0) then
      raise Exception.Create('Informe a Versão na Aba Analista!');
  end;
...
begin
  ...
  begin
    if FController.Model.CDSOcorrenciaRegraConsultaSOcor_Data.AsDateTime < FController.Model.CDSCadastroSol_Data.AsDateTime then
      raise Exception.Create('Data da Ocorrência Regra menor que Data de Abertura!');
  end;
...
begin
  inherited;
  if FController.Model.CDSConsulta.IsEmpty then
    raise Exception.Create('Não há Itens!');
  ...
  try
    ...
    try
      ...
      if Status.Model.CDSCadastroSta_Id.AsInteger = 0 then
        raise Exception.Create('Informe o Status para abertura do chamado nos Parâmetros do Sistema!');
...
//      raise Exception.Create('Salve ou Cancele o Ocorrência Geral !');
//    end;
//  end;
...
//      raise Exception.Create('Salve ou Cancele o Ocorrência Técnica !');
//    end;
//  end;
...
begin
  ...
  begin
    if FController.Model.CDSCronogramaConsultaSCro_Data.AsDateTime < FController.Model.CDSCadastroSol_Data.AsDateTime then
      raise Exception.Create('Data de Previsão de Início menor que Data de Abertura!');
  end;
  ...
  begin
    ...
    begin
      if FController.Model.CDSOcorrenciaGeralConsultaSOcor_Data.AsDateTime < FController.Model.CDSCadastroSol_Data.AsDateTime then
        raise Exception.Create('Data da Ocorrência Geral menor que Data de Abertura!');
    end;
...
begin
  ...
  begin
    if FController.Model.CDSOcorrenciaTecnicaConsultaSOcor_Data.AsDateTime < FController.Model.CDSCadastroSol_Data.AsDateTime then
      raise Exception.Create('Data da Ocorrência Técnica menor que Data de Abertura!');
  end;
