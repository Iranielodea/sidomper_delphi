...
uses
  ...
  FireDAC.Comp.Client, Data.DBXFirebird, Data.FMTBcd, Data.SqlExpr,
  Datasnap.DBClient, Datasnap.Provider, System.Generics.Collections, uDMMSS;
...
begin
  if DMMSS.Conexao.InTransaction then
    DMMSS.Conexao.Commit;
end;

function TFireDAC.Conexao: TFDConnection;
begin
  Result := DMMSS.Conexao;
end;
...
begin
  ...
  Result := DMMSS.Conexao.ExecSQLScalar(sInstrucao);
end;
...
begin
  ...
  DMMSS.Conexao.ExecSQL(sInstrucao);
end;
...
begin
  if not DMMSS.Conexao.InTransaction then
    DMMSS.Conexao.StartTransaction;
end;
